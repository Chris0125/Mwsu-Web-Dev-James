...
<div class="modal hide" id="pleaseWaitDialog" data-backdrop="static" data-keyboard="false">
	...
	<!-- line modal -->
<div class="modal fade" id="modallion" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
		...
		<div class="modal-body">
			...
			<form>
			  ...
			  <div class="form-group">
                ...
                <input type="text" class="form-control" id="img" value="">
              </div>
		...
		<div class="modal-footer">
			<div class="btn-group btn-group-justified" role="group" aria-label="group button">
				...
				<div class="btn-group btn-delete hidden" role="group">
					<button type="button" id="delImage" class="btn btn-default btn-hover-red"   data-toggle="modal" data-target="#commodal" role="button">Delete</button>
				</div>
				<div class="btn-group" role="group">
...
<div class="modal fade" id="commodal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
  <div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
		...
		<div class="modal-footer">
			<div class="btn-group btn-group-justified" role="group" aria-label="group button">
				...
				<div class="btn-group btn-delete hidden" role="group">
					<button type="button" id="delImage" class="btn btn-default btn-hover-red" data-dismiss="modal"  role="button">No</button>
				</div>
				<div class="btn-group" role="group">
					<button type="button" id="saveImage" class="btn btn-default btn-hover-green" data-action="save" role="button">Yes</button>
				</div>
			</div>
		</div>
	</div>
...
(function($) {
    ...
    */
    ...
    function loadTableData(page, page_size,sort,order) {
		...
		console.log("http://107.170.129.183/api/api.php/products?order="+sort+"&page=" + page + "," + page_size);
        $.get("http://107.170.129.183/api/api.php/products?order="+sort+order+"&page=" + page + "," + page_size)
        ...
        .done(function(data) {
            ...
            for (var i = 0; i < records.length; i++) {
                ...
                for (var j = 0; j < records[i].length; j++) {
                    if(j == records[i].length-1){
                        ...
                        records[i][j] = "<img src="+img+".jpg>";
                    }
                    rows = rows + "<td>" + records[i][j] + "</td>";
                }
                rows = rows + '<td style="vertical-align:middle" nowrap><i class="fa fa-pencil-square-o" aria-hidden="true"></i> <i class="fa fa-trash-o trash" data-toggle="modal" data-target="#modallion" aria-hidden="true"></i></td>';
			...
			$('.trash').click(function(){
				var str = $(this).context['parentElement']['parentElement']['innerHTML'];
				str = str.split("<td>");
				console.log(str);
				$('.modallion').id.value = 5;
    ...
    function getTotalPages(){
        $.get("http://107.170.129.183/api/api.php/products")
