...
<script>
$.get( "http://107.170.129.183//api/api.php/products?order=id&page=1,10")
  .done(function( data ) {
    console.log( data );
	...
	var row = data.products.records;
	var line = "<tr>";
	for(var j=0;j<row.length;j++){
	for(var i=0;i<cols.length;i++){
		if( i == 4)
			line = line + '<td><img src="'+ row[j][i].slice(0,-1) + '50_.jpg"> </td>';
		else
			line = line + "<td>" + row[j][i] + "</td>";
	}
	line = line + "</tr>";
	}
	console.log(line);
